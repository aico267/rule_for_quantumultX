...
// 提取倍率识别的核心正则，确保 nx 和 bl 逻辑同步
const blRegex = /((倍率|X|x|×)\D?((\d{1,3}\.)?\d+)\D?)|((\d{1,3}\.)?\d+)(倍|X|x|×)/i;
...
const rurekey = {
  ...
  G: /\d\s?GB/gi,
  Esnc: /esnc/gi,
  中国: /北京|天津|上海|重庆|河北|山西|辽宁|吉林|黑龙江|江苏|浙江|安徽|福建|江西|山东|河南|湖北|湖南|广东|海南|四川|贵州|云南|陕西|甘肃|青海|内蒙古|广西|西藏|宁夏|新疆|石家庄|唐山|秦皇岛|邯郸|邢台|保定|张家口|承德|廊坊|衡水|太原|大同|阳泉|长治|晋城|朔州|晋中|运城|忻州|临汾|吕梁|沈阳|大连|鞍山|抚顺|本溪|丹东|锦州|营口|阜新|辽阳|盘锦|铁岭|朝阳|葫芦岛|长春|吉林市|四平|辽源|通化|白山|松原|白城|哈尔滨|齐齐哈尔|鸡西|鹤岗|双鸭山|大庆|伊春|佳木斯|七台河|牡丹江|黑河|绥化|南京|无锡|徐州|常州|苏州|南通|连云港|淮安|盐城|扬州|镇江|泰州|宿迁|杭州|宁波|温州|嘉兴|湖州|绍兴|金华|衢州|舟山|台州|丽水|合肥|芜湖|蚌埠|淮南|马鞍山|淮北|铜陵|安庆|黄山|滁州|阜阳|宿州|六安|亳州|池州|宣城|福州|厦门|莆田|三明|泉州|漳州|南平|龙岩|宁德|南昌|景德镇|萍乡|九江|新余|鹰潭|赣州|吉安|宜春|抚州|上饶|济南|青岛|淄博|枣庄|东营|烟台|潍坊|济宁|泰安|威海|日照|临沂|德州|聊城|滨州|菏泽|郑州|开封|洛阳|平顶山|安阳|鹤壁|新乡|焦作|濮阳|许昌|漯河|三门峡|南阳|商丘|信阳|周口|驻马店|武汉|黄石|十堰|宜昌|襄阳|鄂州|荆门|孝感|荆州|黄冈|咸宁|随州|长沙|株洲|湘潭|衡阳|邵阳|岳阳|常德|张家界|益阳|郴州|永州|怀化|娄底|广州|韶关|深圳|珠海|汕头|佛山|江门|湛江|茂名|肇庆|惠州|梅州|汕尾|河源|阳江|清远|东莞|中山|潮州|揭阳|云浮|海口|三亚|三沙|儋州|成都|自贡|攀枝花|泸州|德阳|绵阳|广元|遂宁|内江|乐山|南充|眉山|宜宾|广安|达州|雅安|巴中|资阳|贵阳|六盘水|遵义|安顺|昆明|曲靖|玉溪|保山|昭通|丽江|普洱|临沧|拉萨|日喀则|昌都|林芝|山南|那曲|西安|铜川|宝鸡|咸阳|渭南|延安|汉中|榆林|安康|商洛|兰州|嘉峪关|金昌|白银|天水|武威|张掖|平凉|酒泉|庆阳|定西|陇南|西宁|海东|银川|石嘴山|吴忠|固原|中卫|乌鲁木齐|克拉玛依|吐鲁番|哈密|昌吉州|博尔塔拉州|巴音郭楞州|阿克苏|喀什|和田|伊犁州|塔城|阿勒泰/gi,
};
...
function operator(pro) {
  ...
  if (clear || nx || blnx || key) {
    pro = pro.filter((res) => {
      ...
      // 针对 nx 的逻辑优化：复用 blRegex 进行精准匹配
      let isHighMultiplier = false;
      const mMatch = resname.match(blRegex);
      if (mMatch) {
        const valStr = mMatch[0].match(/(\d[\d.]*)/)[0];
        if (parseFloat(valStr) !== 1) isHighMultiplier = true;
      }
      // 补充检查特殊标识
      if (/高倍|ˣ[²³⁴⁵⁶⁷⁸⁹¹⁰]/.test(resname)) isHighMultiplier = true;

      const shouldKeep =
        !(clear && nameclear.test(resname)) &&
        !(nx && isHighMultiplier) && // 这里替换了原来的 namenx.test
  ...
  pro.forEach((e) => {
    ...
    // 正则 匹配倍率 (此处已改为引用 blRegex 常量)
    if (bl) {
      const match = e.name.match(blRegex);
      if (match) {
