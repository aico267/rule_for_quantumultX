 ...
 * [nx]    保留1倍率节点，移除高倍率。
 * [bl]    给节点加倍率
 * [clear] 清理节点名中包含 套餐/官网/剩余流量/订阅 等字样的节点。
 * [key]   只保留 港、台、新、日、美、韩、土 地区。
 * [blgd]  给节点名后面添加倍率对应的关键字，例如：2.0x 对应 2× 
 * [blpx]  给节点按照倍率排序，放在最后
 * [blnx]  只保留高倍率节点
 * [blkey=] 重命名节点名后再次提取关键字：例如：[blkey=BGP+1.0x] 表示提取节点原名中包含 BGP 和 1.0x 的内容。
 * [one] 清理只有一个节点的地区的01。
 */
...
// keya 重新添加省份关键词白名单
const keya = /港|Hong|HK|新加坡|SG|Singapore|日本|Japan|JP|美国|United States|US|韩|土耳其|TR|Turkey|Korea|KR|🇸🇬|🇭🇰|🇯🇵|🇺🇸|🇰🇷|🇹🇷|北京|上海|广州|广东|江苏|浙江|四川|河南/i;
...
function operator(pro) {
  ...
  pro.forEach((e) => {
    ...
    Object.keys(cityToProvince).forEach((prov) => {
      if (cityToProvince[prov].test(e.name)) {
        e.name = e.name.replace(cityToProvince[prov], prov);
      }
    });
